...
export class MerlinBridge {
  constructor() {
    // Bungee Exchange API base URL (configurable via env, defaults to public sandbox)
    // Public sandbox: https://public-backend.bungee.exchange (free, keyless access)
    // For custom endpoint: set SOCKET_API_URL in .env
    // Note: Remove trailing slash and /v2 prefix as we add paths explicitly
    let baseUrl = process.env.SOCKET_API_URL || 'https://public-backend.bungee.exchange';
    
    // Remove trailing slash
    baseUrl = baseUrl.endsWith('/') ? baseUrl.slice(0, -1) : baseUrl;
    
    // Remove /v2 if present (legacy Socket API path)
  ...
  async bridgeSolanaToEvm(params, keypair, fromChainId, toChainId, normalizedToChain) {
    ...
    try {
      ...
      console.log('🌉 Getting bridge quote from Bungee Exchange...');
      console.log('🔗 Base URL:', this.socketApiUrl);

      // Step 1: Get bridge quote from Bungee Exchange API
      // Bungee Exchange public sandbox uses: /api/v1/bungee/quote
      // Ensure proper URL construction: base + '/api/v1/bungee/quote?params...'
      ...
      const quoteUrl = `${this.socketApiUrl}/api/v1/bungee/quote?${queryParams.toString()}`;

      // API key is optional - only include if set
      const apiKey = process.env.SOCKET_API_KEY;
      const headers = {};
      if (apiKey) {
        headers['API-KEY'] = apiKey;
        headers['x-api-key'] = apiKey; // Try both header formats
      }
      ...
      if (!quoteResponse.ok) {
        ...
        console.error('❌ Bungee Exchange quote error:', errorText);
      ...
      // Bungee Exchange public sandbox uses: /api/v1/bungee/build-tx
      const buildTxUrl = `${this.socketApiUrl}/api/v1/bungee/build-tx`;

      const buildTxHeaders = {
        'Content-Type': 'application/json',
      };
      if (apiKey) {
        buildTxHeaders['API-KEY'] = apiKey;
        buildTxHeaders['x-api-key'] = apiKey; // Try both header formats
      }
  ...
  async getBridgeStatus(txHash) {
    try {
      // Check bridge status via Bungee Exchange API
      // Bungee Exchange public sandbox uses: /api/v1/bungee/bridge-status
      const statusUrl = `${this.socketApiUrl}/api/v1/bungee/bridge-status?txHash=${txHash}`;
