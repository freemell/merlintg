...
export class MerlinBridge {
  ...
  async bridgeSolanaToEvm(params, keypair, fromChainId, toChainId, normalizedToChain) {
    ...
    try {
      ...
      let queryParams;
      if (this.isBungeePublic) {
        // Bungee API parameter names - include enableManual=true for Solana bridges
      ...
      if (quote.result) {
        ...
        if (quote.result.autoRoute) {
          selectedRoute = quote.result.autoRoute;
          console.log('✅ Found autoRoute');
        }
        // If no autoRoute, check manualRoutes (populated when enableManual=true)
        else if (quote.result.manualRoutes && Array.isArray(quote.result.manualRoutes) && quote.result.manualRoutes.length > 0) {
          // Select the first route, or you could sort by estimatedOutput/fees
          selectedRoute = quote.result.manualRoutes[0];
          console.log(`✅ Found ${quote.result.manualRoutes.length} manual route(s), using first: ${selectedRoute.name || selectedRoute.integrator || 'Unknown'}`);
        }
        // Legacy: check for routes array
        else if (quote.result.routes && Array.isArray(quote.result.routes) && quote.result.routes.length > 0) {
          selectedRoute = quote.result.routes[0];
          console.log('✅ Found routes array');
        }
        // Legacy: result might be an array
        else if (Array.isArray(quote.result) && quote.result.length > 0) {
          selectedRoute = quote.result[0];
      ...
      if (!selectedRoute) {
        ...
        throw new Error('No bridge routes available for this transaction. The API returned a response but no valid routes were found. This might mean the route is not available, the amount is too small, or the parameters are incorrect.');
      }
