...
export class MerlinBridge {
  constructor() {
    // Socket.tech API base URL (configurable via env, defaults to production)
    // For devnet: https://api-evmx-devnet.socket.tech
    // For production: https://api.socket.tech/v2
    this.socketApiUrl = process.env.SOCKET_API_URL || 'https://api.socket.tech/v2';
  }
  ...
  async bridgeSolanaToEvm(params, keypair, fromChainId, toChainId, normalizedToChain) {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error('SOCKET_API_KEY environment variable is required. Please set it in your .env file. Get your free API key at https://docs.socket.tech/socket-api');
      }
      ...
      if (!quoteResponse.ok) {
        ...
        if (quoteResponse.status === 401) {
          throw new Error('Socket API authentication failed. Please check your SOCKET_API_KEY in .env file. Get your API key at https://docs.socket.tech/socket-api');
        }
  ...
  async getBridgeStatus(txHash) {
    try {
      const apiKey = process.env.SOCKET_API_KEY;
      if (!apiKey) return null; // Skip status check if no API key
