...
export class MerlinBridge {
  constructor() {
    // Bungee Exchange public backend (free, keyless access for testing)
    // Default: https://public-backend.bungee.exchange (shared RPS, not suitable for production)
    // For custom endpoint: set SOCKET_API_URL in .env
    // Note: Remove trailing slash as we add paths explicitly
    let baseUrl = process.env.SOCKET_API_URL || 'https://public-backend.bungee.exchange';
    
    // Remove trailing slash
    baseUrl = baseUrl.endsWith('/') ? baseUrl.slice(0, -1) : baseUrl;
    
    // If base URL doesn't have /v2, add it for Socket API compatibility (for non-Bungee endpoints)
    if (!baseUrl.includes('/v2') && !baseUrl.includes('bungee.exchange')) {
      if (!baseUrl.endsWith('/v2')) {
        baseUrl = baseUrl + '/v2';
      }
    }
    
    this.socketApiUrl = baseUrl;
    this.isBungeePublic = baseUrl.includes('public-backend.bungee.exchange');
  }
  ...
  getTokenAddress(chain, token) {
    ...
    // For native tokens (ETH, BNB, MATIC, etc.), use the standard native token address
    if (tokenUpper === 'SOL' || tokenUpper === 'ETH' || tokenUpper === 'BNB' || tokenUpper === 'MATIC' || tokenUpper === 'AVAX') {
      return '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE'; // Standard native token address for EVM chains
    }
    
    return evmTokens[tokenUpper] || '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE'; // Default to native token address
  }
  ...
  async bridgeSolanaToEvm(params, keypair, fromChainId, toChainId, normalizedToChain) {
    ...
    try {
      ...
      // API key is optional for Bungee public backend, required for Socket.tech
      const apiKey = process.env.SOCKET_API_KEY;
      const headers = {};
      if (apiKey) {
        headers['X-API-Key'] = apiKey; // Socket.tech uses X-API-Key header
        headers['API-KEY'] = apiKey; // Fallback format
        headers['x-api-key'] = apiKey; // Another fallback format
      } else if (!this.isBungeePublic) {
        // Socket.tech API requires authentication (Bungee public backend doesn't)
        throw new Error('SOCKET_API_KEY environment variable is required for Socket.tech API. Get your free API key at https://docs.socket.tech/socket-api or use Bungee public backend (https://public-backend.bungee.exchange)');
      }
      ...
      console.log('✅ Got bridge quote:', JSON.stringify(quote, null, 2));

      // Handle different response formats (Bungee Exchange vs Socket)
      let routes = null;
      if (quote.result && quote.result.routes) {
        routes = quote.result.routes;
      } else if (quote.routes) {
        routes = quote.routes;
      ...
      const route = routes[0]; // Use best route
      const buildTxEndpoint = this.isBungeePublic ? '/api/v1/bungee/build-tx' : '/build-tx';
      ...
      if (apiKey) {
        buildTxHeaders['X-API-Key'] = apiKey; // Socket.tech uses X-API-Key header
        buildTxHeaders['API-KEY'] = apiKey; // Fallback format
        buildTxHeaders['x-api-key'] = apiKey; // Another fallback format
      }
  ...
  async getBridgeStatus(txHash) {
    try {
      ...
      // Bungee public backend doesn't require API key
