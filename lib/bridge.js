...
export class MerlinBridge {
  ...
  getChainId(chain) {
    const chainMapping = {
      ...
      'avalanche': 43114,
      'bsc': 56,
      'bnb': 56, // BNB chain is same as BSC
    };
  ...
  getTokenAddress(chain, token) {
    ...
    const evmTokens = {
      'USDC': chain.toLowerCase() === 'ethereum' ? '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48' :
              chain.toLowerCase() === 'base' ? '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913' :
              chain.toLowerCase() === 'polygon' ? '0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174' :
              chain.toLowerCase() === 'arbitrum' ? '0xaf88d065e77c8cC2239327C5EDb3A432268e5831' :
              '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48',
      'USDT': chain.toLowerCase() === 'ethereum' ? '0xdAC17F958D2ee523a2206206994597C13D831ec7' :
              chain.toLowerCase() === 'polygon' ? '0xc2132D05D31c914a87C6611C10748AEb04B58e8F' :
              '0xdAC17F958D2ee523a2206206994597C13D831ec7',
    };
    
    return evmTokens[tokenUpper] || '0x0000000000000000000000000000000000000000'; // Native token (ETH, MATIC, etc.)
  }
    ...
    try {
      ...
      if (!fromChainId || !toChainId) {
        return {
          ...
          error: `Chain not supported: fromChain=${fromChain}, toChain=${toChain}`
        };
      }

      // For Solana to EVM bridges, use Socket API
      if (fromChain.toLowerCase() === 'solana' && this.isEvmChain(toChain)) {
        return await this.bridgeSolanaToEvm(params, keypair, fromChainId, toChainId);
      }
  ...
  async bridgeSolanaToEvm(params, keypair, fromChainId, toChainId) {
    ...
    try {
      ...
      const quoteUrl = `${this.socketApiUrl}/quote?` + new URLSearchParams({
        ...
        toTokenAddress: this.getTokenAddress(params.toChain, token || 'SOL'),
      ...
      return {
        ...
        message: `✅ Bridge transaction initiated! ${amount} ${token || 'SOL'} bridging from Solana to ${params.toChain}. Transaction: ${signature}`,
  ...
  isEvmChain(chain) {
    const evmChains = ['ethereum', 'base', 'polygon', 'arbitrum', 'optimism', 'avalanche', 'bsc', 'bnb'];
  ...
  async getBridgeQuote(params) {
    ...
    if (['ethereum', 'base'].includes(toChain.toLowerCase())) {
      estimatedTime = '3-5 minutes';
    } else if (['polygon', 'avalanche', 'bsc', 'bnb'].includes(toChain.toLowerCase())) {
