  ...
  async bridgeSolanaToEvm(params, keypair, fromChainId, toChainId, normalizedToChain) {
    ...
    try {
      ...
      if (token === 'SOL' || !token) {
        ...
        // Minimum amount check for bridging (0.1 SOL to cover fees)
        const MIN_SOL_AMOUNT = 0.1;
        if (amountNum < MIN_SOL_AMOUNT) {
          return {
            success: false,
            message: `Amount too small for bridging`,
            error: `Minimum amount for bridging is ${MIN_SOL_AMOUNT} SOL (~$${(MIN_SOL_AMOUNT * 150).toFixed(2)} USD) to cover cross-chain fees. You requested ${amountNum} SOL.`
          };
        }
      ...
      if (quote.result) {
        ...
        // If no autoRoute, check manualRoutes (populated when enableManual=true)
          ...
          // Sort routes by estimatedOutput (highest first) or lowest fees
          const sortedRoutes = result.manualRoutes.sort((a, b) => {
            const outputA = parseFloat(a.estimatedOutput || a.outputAmount || 0);
            const outputB = parseFloat(b.estimatedOutput || b.outputAmount || 0);
            return outputB - outputA; // Highest output first
          });
          selectedRoute = sortedRoutes[0];
          console.log(`✅ Found ${result.manualRoutes.length} manual route(s), using best: ${selectedRoute.name || selectedRoute.integrator || 'Unknown'}`);
        }
      ...
      if (!selectedRoute) {
        ...
        if (hasAutoRoute === false && manualRoutesCount === 0 && !hasDepositRoute) {
          ...
          errorMessage += '- Amount too small (minimum ~0.1 SOL / ~$15 USD to cover fees)\n';
          errorMessage += '- Route not supported for this token pair\n';
          errorMessage += '- Low liquidity for this specific combination\n';
          errorMessage += `\nYou requested: ${amountNum || 'unknown'} SOL. Try increasing to at least 0.1 SOL.`;
        }
