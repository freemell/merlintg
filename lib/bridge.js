...
export class MerlinBridge {
  constructor() {
    // Socket.tech API base URL (configurable via env, defaults to Socket public API)
    // Socket.tech public API: https://api.socket.tech/v2 (free, keyless access for basic usage)
    // Bungee Exchange public sandbox: https://public-backend.bungee.exchange (may have limited endpoints)
    // For custom endpoint: set SOCKET_API_URL in .env
    // Note: Remove trailing slash as we add paths explicitly
    let baseUrl = process.env.SOCKET_API_URL || 'https://api.socket.tech/v2';
    
    // Remove trailing slash
    baseUrl = baseUrl.endsWith('/') ? baseUrl.slice(0, -1) : baseUrl;
    
    // If base URL doesn't have /v2, add it for Socket API compatibility
    if (!baseUrl.includes('/v2') && !baseUrl.includes('/api/v1/bungee')) {
      // If it's the Bungee public backend, use their endpoint structure
      if (baseUrl.includes('bungee.exchange')) {
        // Keep as-is for Bungee endpoints
      } else {
        // For Socket API, ensure /v2 is present
  ...
  async bridgeSolanaToEvm(params, keypair, fromChainId, toChainId, normalizedToChain) {
    ...
    try {
      ...
      let quoteEndpoint;
      if (this.socketApiUrl.includes('bungee.exchange')) {
        // Bungee Exchange uses: /api/v1/bungee/quote
        quoteEndpoint = '/api/v1/bungee/quote';
      } else {
      ...
      // API key is required for Socket.tech API
      ...
      if (apiKey) {
        ...
        // Socket.tech API requires authentication
        throw new Error('SOCKET_API_KEY environment variable is required for bridge functionality. Get your free API key at https://docs.socket.tech/socket-api');
      }
      ...
      let buildTxEndpoint;
      if (this.socketApiUrl.includes('bungee.exchange')) {
        buildTxEndpoint = '/api/v1/bungee/build-tx';
      } else {
        buildTxEndpoint = '/build-tx';
      }
  ...
  async getBridgeStatus(txHash) {
    try {
      ...
      let statusEndpoint;
      if (this.socketApiUrl.includes('bungee.exchange')) {
        statusEndpoint = '/api/v1/bungee/bridge-status';
      } else {
        statusEndpoint = '/bridge-status';
      }
