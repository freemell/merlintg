...
import { Connection, PublicKey, VersionedTransaction, LAMPORTS_PER_SOL, TransactionInstruction, TransactionMessage, Keypair } from '@solana/web3.js';
  ...
  async buildSolanaTransaction(txData, keypair, toAddress) {
    try {
      ...
      if (txData.instructions && Array.isArray(txData.instructions)) {
        ...
        console.log(`✍️ Transaction signed with ${signers.length} signer(s)`);
        
        // Send transaction with skipPreflight for speed (monitor for errors)
        const signature = await connection.sendTransaction(transaction, signers, {
          skipPreflight: true, // Optional for speed, but monitor
          maxRetries: 3,
        });
