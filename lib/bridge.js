...
export class MerlinBridge {
  ...
  async bridgeSolanaToEvm(params, keypair, fromChainId, toChainId, normalizedToChain) {
    ...
    try {
      ...
      // Bungee Exchange public sandbox uses Socket API format: /v2/quote
      const quoteUrl = `${this.socketApiUrl}/v2/quote?` + new URLSearchParams({
      ...
      // Bungee Exchange public sandbox uses Socket API format: /v2/build-tx
      const buildTxUrl = `${this.socketApiUrl}/v2/build-tx`;
  ...
  async getBridgeStatus(txHash) {
    try {
      ...
      // Bungee Exchange public sandbox uses Socket API format: /v2/bridge-status
      const statusUrl = `${this.socketApiUrl}/v2/bridge-status?txHash=${txHash}`;
