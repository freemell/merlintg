...
import { Connection, PublicKey, VersionedTransaction, LAMPORTS_PER_SOL, TransactionInstruction, TransactionMessage, Keypair, Transaction } from '@solana/web3.js';
...
export class MerlinBridge {
  ...
  async ensureAtas(connection, keypair, mintAddresses = []) {
  ...
  async bridgeWithMayan(params, keypair, normalizedToChain, amountInSmallestUnit) {
    ...
    logger.info('Selected Mayan quote', { route: bestQuote.routeName || 'unknown', toChain: normalizedToChain });

    // Ensure WSOL ATA exists prior to swap
    await this.ensureAtas(connection, keypair, ['So11111111111111111111111111111111111111112']);
    ...
    const signature = swapResult?.signature || swapResult?.txHash || swapResult;
    if (!signature) {
      throw new Error('Mayan fallback returned invalid response');
    }

    logger.info('Mayan fallback bridge submitted', { signature });
  ...
  async bridgeSolanaToEvm(params, keypair, fromChainId, toChainId, normalizedToChain) {
    ...
    try {
      ...
      await this.ensureAtas(connection, keypair, Array.from(mintCandidates));
