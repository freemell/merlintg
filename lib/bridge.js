...
export class MerlinBridge {
  ...
  getTokenAddress(chain, token) {
    ...
    if (chainLower === 'solana') {
      ...
      // For native SOL on Solana, return Solana native token address
      if (tokenUpper === 'SOL' || !tokenUpper) {
        return solanaTokens['SOL'];
      }
    ...
    // For native tokens (SOL, ETH, BNB, MATIC, AVAX, etc.), use lowercase native token address
    if (tokenUpper === 'SOL' || tokenUpper === 'ETH' || tokenUpper === 'BNB' || tokenUpper === 'MATIC' || tokenUpper === 'AVAX' || !tokenUpper) {
      return '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'; // Native token address for EVM chains (lowercase)
    }
    
    // Return mapped token address or default to native token address
    return evmTokens[tokenUpper] || '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee';
  }
  ...
  async bridgeSolanaToEvm(params, keypair, fromChainId, toChainId, normalizedToChain) {
    ...
    try {
      ...
      let queryParams;
      if (this.isBungeePublic) {
        // Bungee API parameter names - using only required parameters
        ...
        // Optional parameters - only add if they're commonly supported
        // Some APIs might reject unknown parameters
      } else {
      ...
      if (quote.result) {
        // Check if result has routes
        if (quote.result.routes) {
          routes = quote.result.routes;
        } else if (Array.isArray(quote.result)) {
          routes = quote.result;
        } else if (quote.result.result && quote.result.result.routes) {
          routes = quote.result.result.routes;
        } else if (quote.result.data && quote.result.data.routes) {
          routes = quote.result.data.routes;
        }
      } else if (quote.routes) {
        routes = quote.routes;
      } else if (Array.isArray(quote)) {
        routes = quote;
      } else if (quote.data && quote.data.routes) {
        routes = quote.data.routes;
      } else if (quote.data && Array.isArray(quote.data)) {
        routes = quote.data;
      }

      console.log('🔍 Extracted routes:', routes ? `${routes.length} route(s) found` : 'No routes found');
      console.log('🔍 Routes data:', routes ? JSON.stringify(routes, null, 2) : 'null');
      ...
      if (!routes || routes.length === 0) {
        ...
        throw new Error('No bridge routes available for this transaction. The API returned a response but no valid routes were found. This might mean the route is not available or the parameters are incorrect.');
      }
      ...
      // No authentication headers for Bungee public backend
