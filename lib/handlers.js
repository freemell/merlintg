...
import { sendSOLTransaction, getBalance, swapTokens } from './solana.js';
...
export async function handleAIAction(ctx, aiResult, session, setSessionFn) {
  ...
  // First, always send the AI response (escape usernames to prevent Markdown parsing errors)
  ...
  switch (action) {
    ...
    case 'swap':
      if (params.fromToken && params.toToken && params.amount) {
        ...
        // Execute swap
        const result = await swapTokens(keypair, params.fromToken, params.toToken, params.amount);
        ...
        if (result.success && result.signature) {
          ...
          message += `💰 Swapped: *${result.quote?.inputAmount?.toFixed(4) || params.amount} ${params.fromToken}*\n`;
        ...
        await ctx.reply('❌ Missing swap parameters. Please provide fromToken, toToken, and amount.', {
          reply_markup: isGroup ? undefined : getMainMenu()
        });
      }
      break;
    ...
    default:
      // For other actions (bridge, tx, etc.), just show the AI response
...
export async function handleAIMessage(ctx, messageText, session, setSessionFn) {
  try {
    ...
    const aiResult = await processMessageWithAI(messageText);
